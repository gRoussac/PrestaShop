...
class HTMLTemplateInvoiceCore extends HTMLTemplate
{
    ...
    */
	...
	public function getFilename()
	{
		...
		if (Configuration::get('PS_INVOICE_YEAR'))
			$format = Configuration::get('PS_INVOICE_YEAR_POS') ? '%1$s%3$s-%2$06d' : '%1$s%2$06d-%3$s';

		return sprintf($format, Configuration::get('PS_INVOICE_PREFIX', $id_lang, null, $id_shop), $this->order_invoice->number, date('Y', strtotime($this->order_invoice->date_add)));
	}
}
